...
export default function Profile({
   ...
   */
  const glance: { value: string; label: Pair }[] = [
  ...
  const bar = (s: StackRow) => (
    <div key={s.name} className={`${ROW} border-t border-rule py-2.5`}>
      <span className={`text-[14px] lg:text-[16px] ${lang === "jp" ? "font-jp" : ""}`}>
        {s.name}
      </span>
      <span
        // Bars scale from the left; the number beside them is printed from
        // frame 0, so the bar is never the only thing carrying the value.
        style={{ width: `${(s.months / AXIS_MONTHS) * 100}%` }}
        className={`block h-3.5 ${band(s.months)}`}
      />
  ...
  const bandHead = (label: Pair, unit?: boolean) => (
    <div className={`${ROW} border-t border-g5 py-2.5`}>
      <span
  ...
  return (
    <section id="profile" className="px-5 py-14 lg:px-24 lg:pb-[72px] lg:pt-16">
      <div className="mx-auto w-full max-w-content">
        {/* Header */}
        <div className="grid items-end gap-8 border-b-[3px] border-bone pb-7 lg:grid-cols-[1fr_400px] lg:gap-16">
          <div>
          ...
          <div className="group flex items-end gap-5">
        ...
        {/* Glance strip — the five-second read */}
        <div className="mt-8 grid grid-cols-2 gap-8 border-b border-g5 pb-8 lg:mt-10 lg:grid-cols-4 lg:gap-12 lg:pb-9">
          {glance.map((g) => (
        ...
        <div className="mt-10 grid gap-12 lg:mt-11 lg:grid-cols-[1fr_620px] lg:gap-[88px]">
          {/* — Bio + track — */}
          <div>
            <div className="font-mono text-label text-g6">
            ...
            <div className="mt-10 font-mono text-label text-g6 lg:mt-11">
            ...
            <div className="mt-4 flex flex-col">
              {phases.map((p) => (
                <button
                  ...
                  className={`group grid grid-cols-[72px_1fr] gap-5 border-t py-4 text-left transition-colors duration-flick ease-snap lg:grid-cols-[96px_1fr] lg:gap-6 ${
                    p.now
          ...
          <div>
            <div className="flex justify-between gap-5 font-mono text-label text-g6">
            ...
            <div className="mt-4 flex flex-col">
              ...
              {stack.filter((s) => s.group === "daily").map(bar)}
              {bandHead(ui.profile.groupProduct)}
              {stack.filter((s) => s.group === "product").map(bar)}
